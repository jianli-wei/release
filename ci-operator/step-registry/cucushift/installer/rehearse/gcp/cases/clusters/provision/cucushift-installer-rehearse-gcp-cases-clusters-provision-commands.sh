...
echo "$(date -u --rfc-3339=seconds) - Check AMI"
echo "$(date -u --rfc-3339=seconds) - skipped"
...
echo "$(date -u --rfc-3339=seconds) - Patch architecture"
if [ "${ARCH}" == "" ]; then
...
do
  # On considering the machine-types describe command is time consuming,  
  # and generally we use the same machine series/families for control-plane and compute, 
  # only check for the control-plane machine type availability
  if gcloud compute machine-types describe "${CONTROL_PLANE_INSTANCE_TYPE}" --zone "${ZONE_NAME}"; then
    ...
    echo "Skip zone '${ZONE_NAME}' for machine type '${CONTROL_PLANE_INSTANCE_TYPE}' / '${COMPUTE_INSTANCE_TYPE}'."
  fi
done
