...
iam_account="ipi-min-permissions-sa@${SA_PROJECT_ID}.iam.gserviceaccount.com"

display_name=$(gcloud --project "${SA_PROJECT_ID}" iam service-accounts list --filter="email=${iam_account}" --format='value(displayName)')
if [[ -z "${display_name}" ]]; then
  echo "$(date -u --rfc-3339=seconds) - Failed to find the IAM service account '${iam_account}' in GCP project '${SA_PROJECT_ID}', abort."
  exit 1
fi

gcloud --project "${SA_PROJECT_ID}" iam service-accounts keys create "${SHARED_DIR}/gcp_min_permissions.json" --iam-account="${iam_account}" || exit 1
