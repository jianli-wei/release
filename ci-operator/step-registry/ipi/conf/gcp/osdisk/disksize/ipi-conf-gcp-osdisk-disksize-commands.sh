...
if [ -z "${COMPUTE_DISK_SIZEGB}" ] && [ -z "${CONTROL_PLANE_DISK_SIZEGB}" ] && [ -z "${DEAFULT_MACHINE_PLATFORM_DISK_SIZEGB}" ]; then
  echo "Empty 'COMPUTE_DISK_SIZEGB' / 'CONTROL_PLANE_DISK_SIZEGB' / 'DEAFULT_MACHINE_PLATFORM_DISK_SIZEGB', nothing to do, exiting."
  exit 0
fi
...
echo "DEAFULT_MACHINE_PLATFORM_DISK_SIZEGB: ${DEAFULT_MACHINE_PLATFORM_DISK_SIZEGB}"
...
if [ -n "${DEAFULT_MACHINE_PLATFORM_DISK_SIZEGB}" ]; then
    cat > "${PATCH}" << EOF
platform:
  gcp:
    defaultMachinePlatform:
      osDisk: 
        diskSizeGB: ${DEAFULT_MACHINE_PLATFORM_DISK_SIZEGB}
EOF
  yq-go m -x -i "${CONFIG}" "${PATCH}"
